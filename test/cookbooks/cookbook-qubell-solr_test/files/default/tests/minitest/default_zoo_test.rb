...
describe_recipe "cookbook-qubell-solr::default" do
  ...
  it "check cores config solr.xml correct" do
    ...
    assert_include_content("#{node["cookbook-qubell-solr"]["path"]}/cores/solr.xml", "zkHost=\"#{node["cookbook-qubell-solr"]["zookeeper"]["nodes"]}\"")
    assert_include_content("#{node["cookbook-qubell-solr"]["path"]}/cores/solr.xml", "zkClientTimeout=\"#{node["cookbook-qubell-solr"]["zookeeper"]["timeout"]}\"")
  end
