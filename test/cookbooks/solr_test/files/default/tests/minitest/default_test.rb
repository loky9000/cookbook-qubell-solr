...
describe_recipe "solr::default" do
  ...
  it "check cores config solr.xml correct" do
    ...
    refute_include_content("#{node["solr"]["path"]}/cores/solr.xml", "zkHost=\"=#{node["solr"]["zookeeper"]["nodes"]}\"")
    refute_include_content("#{node["solr"]["path"]}/cores/solr.xml", "zkClientTimeout=\"#{node["solr"]["zookeeper"]["timeout"]}\"")
  end
